...
--     SELECT app_name, reviews FROM analytics ORDER BY rating DESC LIMIT 5;

-- Find the app that has the most reviews with a rating greater than equal to 4.8.
--     SELECT app_name FROM analytics ORDER BY reviews DESC LIMIT 1;
...
--     SELECT MAX(last_updated),MAX(app_name) FROM analytics;

-- Find the most expensive app (the query is very similar to #11).
--     SELECT MAX(price),MAX(app_name) FROM analytics;
